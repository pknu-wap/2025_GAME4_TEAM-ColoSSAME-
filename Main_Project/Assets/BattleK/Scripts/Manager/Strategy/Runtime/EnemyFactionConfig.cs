...
public class EnemyFactionConfig : ScriptableObject
{
    ...
    {
        int want = Mathf.Max(0, rosterCount);

        if (weightedEnemyKeys != null && weightedEnemyKeys.Count > 0)
        {
    ...
    private List<string> PickWeighted(List<WeightedKey> source, int count, bool withReplacement)
    {
        ...
        if (withReplacement)
        {
            float sum = SumWeights(source);
            if (sum <= 0f) return result;
            for (int i = 0; i < count; i++)
            {
                string pick = SpinRoulette(source, sum);
        ...
        else
        {
            ...
            for (int i = 0; i < count && pool.Count > 0; i++)
            {
                ...
                if (sum <= 0f) break;
                int idx = SpinRouletteIndex(pool, sum);
