...
namespace BattleK.Scripts.UI
{
    public class SlotPositionSyncer : MonoBehaviour
    {
        ...
        private void ApplyPositionsToSlots(Vector2[] positions)
        {
            ...
            var count = Mathf.Min(slots.Length, positions.Length);
            ...
            if (!resetUnusedSlotsToZero || slots.Length <= count) return;
            {
        ...
        public void CopyPreviewToSlots()
        {
            ...
            var count = Mathf.Min(previewSlots.Length, slots.Length);
            for (var i = 0; i < count; i++)
            {
