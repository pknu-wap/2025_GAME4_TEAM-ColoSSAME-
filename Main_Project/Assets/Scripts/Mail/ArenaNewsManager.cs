...
public class ArenaNewsManager : MonoBehaviour
{
        ...
        Dictionary<int, List<char>> cur, Dictionary<int, List<char>> prev)
    {
        ...
        {
            pool.Add(Make(Pick(
                $"{teamA.name}{Wa(teamA.name)} {teamB.name}, 무승부로 승점 1점씩 나누다",
        ...
        bool isPlayer = winner.id == league.settings.playerTeamId || loser.id == league.settings.playerTeamId;
        int pb = isPlayer ? 20 : 0;
        ...
        if (winStreak >= 10)
            pool.Add(Make(Pick(
                $"{winner.name}, 파죽의 {winStreak}연승 — 막을 자가 없다",
                $"{winner.name}, {winStreak}연승 달성으로 리그를 지배하다"),
                100 + pb, winner.id));

        if (winner.rank == 1)
            pool.Add(Make(Pick(
                $"{winner.name}, {loser.name}{Eul(loser.name)} 꺾고 단독 선두 등극",
                $"{winner.name}, 승리와 함께 리그 정상에 서다"),
                100 + pb, winner.id));

        if (winner.rank >= league.teams.Count - 1 && loser.rank <= 2)
            pool.Add(Make(Pick(
                $"최하위 {winner.name}, 선두 {loser.name}{Eul(loser.name)} 꺾는 이변",
        ...
        if (prevLoseStreak >= streakThreshold)
            pool.Add(Make(Pick(
                $"{winner.name}, {prevLoseStreak}연패를 끊고 반격에 나서다",
        ...
        if (prevWinStreak >= streakThreshold)
            pool.Add(Make(Pick(
                $"{loser.name}의 {prevWinStreak}연승 행진이 막을 내리다",
        ...
        if (winStreak == streakThreshold)
            pool.Add(Make(Pick(
                $"{winner.name}, {winStreak}연승으로 기세를 올리다",
        ...
        if (loseStreak == streakThreshold)
            pool.Add(Make(Pick(
                $"{loser.name}, {loseStreak}연패 — 위기에 빠지다",
        ...
        // Score 40
        pool.Add(Make(Pick(
            $"{winner.name}, {loser.name}{Eul(loser.name)} 꺾고 승점을 쌓다",
